...
---------------------
    ...
    BASE_MODEL   – HF model ID of the base model (default: meta-llama/Llama-3.2-3B)
...
def load_model():
    ...
    if adapter_dir.exists():
        print(f"[startup] Loading LoRA adapter from {adapter_dir}")
        ...
        print(f"[startup] Adapter not found at {adapter_dir}. Loading base model: {BASE_MODEL}")
...
async def lifespan(app: FastAPI):
    ...
    if tables_path.exists():
        ...
        print(f"[startup] Loaded Spider schema index ({len(state.spider_index)} databases).")

    # Database resolver
    db_root = Path(DB_ROOT)
    if db_root.exists():
        state.resolver = DatabaseResolver(str(db_root))
        print(f"[startup] DatabaseResolver pointed at {db_root}.")
...
class GenerateRequest(BaseModel):
    question: str = Field(..., description="Natural language question.", example="How many concerts are there?")
    schema: str = Field(
        ...,
        description=(
